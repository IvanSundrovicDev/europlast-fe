      ...
      <div @mouseenter="sidebarMouseEntered" @mouseleave="sidebarMouseLeave">
        ...
        >
          <div class="logo flex items-center">
            <img
              v-if="this.$route.name !== 'kalendar'"
              :src="logo"
              v-on:click="$router.push({ name: 'kalendar' })"
              alt="logo"
              class="w-10 mr-4"
            />
            <img v-else :src="logo" alt="logo" class="w-10 mr-4" />
          </div>
        </div>
        ...
        >
          <template v-for="(sidebarItem, index) in sidebarItems">
            ...
            <span
              :key="`header-${index}`"
              v-if="sidebarItem.header && !sidebarItemsMin && test(sidebarItem)"
              class="navigation-header truncate"
              >{{ sidebarItem.header }}</span
            >
            <template v-else-if="!sidebarItem.header && test(sidebarItem)">
              <!-- IF IT'S SINGLE ITEM -->
              <vx-sidebar-item
                ref="sidebarLink"
...
export default {
  ...
  methods: {
    test(item) {
      if (Object.prototype.hasOwnProperty.call(item, "requiresAdmin")) {
